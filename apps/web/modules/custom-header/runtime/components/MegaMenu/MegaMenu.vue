<template>
  <header ref="referenceRef" :class="headerClass" class="relative w-full md:sticky md:shadow-md z-10">
    <div
      class="flex justify-between items-center gap-6 flex-wrap md:flex-nowrap px-4 md:px-10 py-2 md:py-5 w-full border-0 border-neutral-200"
    ...
    >
      ...
      <div class="flex items-center gap-4 min-w-0">
        <nav v-if="viewport.isGreaterOrEquals('lg')" ref="floatingRef" class="min-w-0">
          <ul
            class="flex flex-nowrap items-center gap-1 overflow-x-auto"
            @blur="
              (event: FocusEvent) => {
          ...
          >
            ...
            >
              ...
              >
                ...
                <template v-else>
                  <NuxtLink :to="localePath(generateCategoryLink(menuNode))" class="flex items-center gap-2 w-full text-white">
                    <span>{{ categoryTreeGetters.getName(menuNode) }}</span>
                  </NuxtLink>
                </template>
              </div>
